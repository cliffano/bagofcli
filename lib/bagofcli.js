...
function command(base, actions, opts) {
  ...
  _validate(commander.args, commands.commands);
}

// Validation is done after commander finished parsing,
// which means command action might already start and will be
// interrupted ungracefully by calling exit.
function _validate(args, commands) {
  if (!args) { return; }

  var programName = args[args.length - 1].parent._name,
    commandName = args[args.length - 1]._name,
    commandArgs = commands[commandName].args;

  if (commandArgs && commandArgs.length > 0) {
    if (args.length === 1) {
      exit(new Error(util.format('Usage: %s %s <%s>', programName, commandName, _.pluck(commandArgs, 'name').join('> <'))));
    } else {
      try {
        function check(arg, commandArg) {
          return function (rule) {
            validator.check(
              arg,
              util.format('Invalid argument: <%s> must be %s', commandArg.name, rule)
            )[rule]();
          };
        }
        for (var i = 0, ln = commandArgs.length; i < ln; i += 1) {
          commandArgs[i].rules.forEach(check(args[i], commandArgs[i]));
        }
      } catch (e) {
        exit(e);
      }
