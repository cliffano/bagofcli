...
function _postCommand(args, commandsConfig, globalOptsConfig) {
  ...
  function _validate(value, name, desc) {
    return function (rule) {
      const _iz = iz({ value: value, validators: validators });
      if (validators.hasOwnProperty(rule)) {
        const isValid = validators[rule](value).valid;
  ...
  if (!args) {
    ...
    if (typeof lastArg !== 'object') {
      ...
      const programName = lastArg.parent._name;

      // display usage info when expected arguments don't exist
      if (args.length === 1) {
        exit(new Error(util.format('Usage: %s %s %s', programName, commandName, commandArgs.map((arg) => {
          return util.format((arg.optional) ? '[%s]' : '<%s>', arg.name);
        }).join(' '))));
      } else {
        // validate arguments as configured in commands setup
        for (let i = 0, ln = commandArgs.length; i < ln; i += 1) {
    ...
    if (commandOpts) {
      commandOpts.forEach((commandOpt) => {
        if (commandOpt.rules) {
