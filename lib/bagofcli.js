...
function command(base, actions, opts) {
  ...
  _.each(commands.commands, function (command, name) {
    const program = commander.command(name).description(command.desc);
...
function _postCommand(commanderArg, commandsConfig, globalOptsConfig) {
  ...
  function _getOptionValue(commandArg, arg) {
    const name = arg.match(/<.*>/)[0].replace(/[<>]/g, "");
    if (
      commandArg &&
      typeof commandArg.opts === "function" &&
      commandArg.opts()[name] !== undefined
    ) {
      return commandArg.opts()[name];
    }
